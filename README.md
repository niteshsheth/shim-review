...
https://www.lei-lookup.com/record/549300LMV4JEXTGLE324
...
subject=C = US, ST = California, L = Aliso Viejo, O = Quest Software Inc., serialNumber = 4645336, CN = Quest Software Inc., businessCategory = Private Organization, jurisdictionST = Delaware, jurisdictionC = US
...
*******************************************************************************
iPXE is an open-source network bootloader that is widely used by enterprises, universities, cloud providers, and individuals to boot operating systems over the network using PXE, HTTP, iSCSI, and other protocols.

Without being signed, end users would be forced to disable Secure Boot or manually enroll custom keys, which is not practical in large-scale or heterogeneous environments that rely on commodity hardware.

Having shim signed by the Microsoft 3rd-party UEFI CA ensures that iPXE can boot out-of-the-box on any Secure Boot–enabled system, regardless of vendor, without requiring manual firmware configuration. This is critical for enabling secure, interoperable, and scalable deployments across diverse hardware platforms worldwide.

*******************************************************************************
### Why are you unable to reuse shim from another distro that is already signed?
*******************************************************************************
It is not possible to reuse shim from another distribution because each shim is built and signed with a certificate that is specific to that distribution. Fedora’s shim, for example, will only trust binaries signed with Fedora’s key. Our binaries (iPXE and related components) are signed with our own EV certificate and therefore require our own shim to establish a valid trust chain.

Reusing another distro’s shim would break the Secure Boot trust model and would also inappropriately delegate responsibility for our binaries to that distribution. To ensure a proper chain of trust and independent maintenance, we require our own signed shim.
...
*******************************************************************************
The kernel is signed with our vendor key and verified via shim’s built-in signature validation. When Secure Boot is enabled, the kernel enforces lockdown mode (CONFIG_LOCK_DOWN_KERNEL) to prevent operations that would undermine Secure Boot (such as unsigned kernel module loading, raw I/O port access, or kexec of unsigned kernels).

iPXE itself is built to rely on shim’s verification, and it does not bypass the Secure Boot trust chain. It chainloads only signed kernels, and those kernels enforce lockdown under Secure Boot.
...
*******************************************************************************
docker build -t shim-builder -f shimdockerfile.ubuntu .
docker run --rm -v $(pwd)/output:/output shim-builder
